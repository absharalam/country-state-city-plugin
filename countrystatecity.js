(function($) {
    ...
    populateCountry = function(element, id, selected = 0){
    	...
    	return $.getJSON("json/countries.json", function(result){
	        $.each(result.countries, function(i, field){
	        	if(selected == field.id)
	        		$("select[id='"+id+"']").append($('<option selected></option>').val(field.id).html(field.name));
	        	else
	            	$("select[id='"+id+"']").append($('<option></option>').val(field.id).html(field.name));
	        });
    ...
    populateState= function(element, country_id){
    	...
    	return $.getJSON("json/states.json", function(result){
	        	...
	        	if(field.country_id == country_id){
	            	...
	            	if(typeof selected_value !== typeof undefined && selected_value !== false){
	            		if(selected_value == field.id){
	            			isSelected = true;
	            			$("#"+element).append($('<option selected></option>').val(field.id).html(field.name));
	            		}
	            		else
	            			$("#"+element).append($('<option></option>').val(field.id).html(field.name));
	            	}else{
	            		$("#"+element).append($('<option></option>').val(field.id).html(field.name));
	            	}
	        	}
    ...
    populateCity = function(element, state_id){
    	...
    	return $.getJSON("json/cities.json", function(result){
